...
* A [design file](%%BASEURL/config/design), `-d /home/me/data/example/anat-antsct.dsn`
* A [cohort file](%%BASEURL/config/cohort), `-c /home/me/data/example/anat_cohort.csv`
* An [output path](%%BASEURL/config/xcpEngine#xcpengine--o-output-path), `/home/me/data/example/xcp_output`
* A [verbosity level](%%BASEURL/config/xcpEngine#xcpengine--t-trace-verbosity-level), `-t 1`
* A [reference directory](%%BASEURL/config/xcpEngine#xcpengine--r-reference-directory), `-r /home/me`
...
* An atlas directory (`sub-001_atlas`). Inside the atlas directory, each parcellation that has been analysed will exist as a NIfTI file, registered to the subject's [ (T1w) native space](%%BASEURL/space).
...
* A spatial metadata file (`sub-001_spaces.json`). The pipeline uses this to determine how to move images between different [coordinate spaces](%%BASEURL/space).
